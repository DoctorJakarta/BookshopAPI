...
public class TagResource {
    ...
    @Path("{tagKey}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getTagById( @PathParam("tagKey") String key) {
		try {
			Tag tag = new TagDAO().getTagByKey(key);
    ...
    @Path("{tagId}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response deleteTagById( @PathParam("tagId") String key) {
		try {
			new TagDAO().deleteTag(key);
	        return Response.ok(null, MediaType.APPLICATION_JSON).build();
		} catch (DatabaseException e) {
			e.printStackTrace();
			return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).entity(e.getErrorResponse()).build();
